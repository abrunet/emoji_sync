...
def emoji_admin_list(batch_size, token):
    def inner_func():
        print("Calling admin list API")
        return requests.post('https://slack.com/api/emoji.adminList', data={
            'query': '',
            'page': '1',
...
def main():
    ...
    source_emoji = request_and_cache_api(SOURCE_EMOJI, emoji_list(args.api_token))
    ...
    source_emoji = source_emoji['emoji']
    ...
    data = request_and_cache_api('admin_list_cache.json',
                                 emoji_admin_list(batch_size, args.xoxs_token))
